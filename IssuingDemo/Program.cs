    ...
    class Program
    {
        ...
        public static async Task GenerateReport()
        {
            ...
            using (var csv = new CsvReader(reader, config))
            {
                /*                while (csv.Read())
                                {
                                    PanelModel p = new PanelModel();
                                    p.PanelType = csv.GetField<string>(0);
                                    Console.WriteLine(p);
                                }
                                records = csv.GetRecords<PanelModel>().ToList();
                */
                ...
                if (intSq.Count > 0)
                {
                   // nie dziala intSq.Select(x => new { x.PanelRef, x.Length, x.Height, x.Area, x.Weight, x.Qty }).ToList();
        ...
        private static async Task SaveExcelFile(IEnumerable<PanelModel> panels, FileInfo file, string wsName)
        {
            ...
            using (var package = new ExcelPackage(file))
            {
                ...
                CreateSummaryMakePanels(ws, wsAreaSum, wsLengthSum);
                CreateHeadersPanelMaking(ws);

                var range = ws.Cells["A14"].LoadFromCollection(panels, false);
        ...
        private static void CreateSummaryMakePanels(ExcelWorksheet ws, double wsAreaSum, double wsLengthSum)
        {
